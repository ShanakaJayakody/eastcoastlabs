...
- [x] Write failing SQL tests: `expect(commit(staleQuote)).rejects.toThrow(); expect(await totalSettled()).toBe(refundedCents)` after concurrent/replayed requests; include discounted partial/final refunds, shipping once, restock false, grants.
- [x] Add SQL quote/commit/ledger with order locks and immutable audit evidence, server wrappers and UI previews/errors. Both full and partial refunds must use previews.
- [x] Run targeted SQL/UI tests and typecheck, inspect diff, record evidence and commit only owned files. Review before next worker task.
...
- [x] Write failing tests for concurrent lot exhaustion, refunded packing quantity, lot/product mismatch, quoted CSV/BOM/newlines/duplicate conflicts and stale tracking preview.
- [x] Implement atomic allocation plus guarded release/reallocation, printable assignments and CSV upload/preview/commit UI using existing status/tracking operations and optional notification intent.
- [x] Run focused tests/typecheck, report and commit owned files. Review before next task.
...
- [x] Write failing tests for legacy/new carts, inactive/mismatched IDs, invalid-field focus, explicit request/confirm/expiry/replay, suppression and restore token privacy.
- [x] Implement consent UI and token lifecycle integrated with existing outbox/episodes; account for private page exclusion and make repeat confirmation harmless.
- [x] Run focused tests/typecheck, report and commit owned files. Review before next task.
...
- [x] Write failing tests for dead/retry timing, active leases, duplicate provider delivery, refund delta replay and transactional product failure.
- [x] Implement narrow guarded controls and atomic utilities using existing outbox/commerce helpers; update operational documentation.
- [x] Run focused tests/typecheck, report and commit owned files. Review before integration.
