...
The commerce shell uses Tenor Sans (400, upright) for display headings and Commissioner (400–700, upright variable) for body text and controls. Both are self-hosted Latin WOFF2 files loaded through `next/font/local` with swap and adjusted fallbacks. Admin and the `/1` experiment retain their existing typography.
...
Inter and Newsreader files from the preceding design remain archived here but are no longer referenced or preloaded by the commerce shell.
