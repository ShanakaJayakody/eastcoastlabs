...
Stock attribution preview: `/frame.html?page=stock-admin&bare=1&open=1` opens the real stock panel with synthetic named receipts, quantities and a reversed receipt. It uses the same disabled stock-write stubs. The browser check verifies attribution is visible without opening a history disclosure, including a latest-receipt summary on mobile, no email addresses in the stock panel, no horizontal overflow, and accessibility.

Dashboard stock ownership preview: `/frame.html?page=stock-dashboard-admin&bare=1` shows the real dashboard component with synthetic receipts and sales processed through the FIFO allocation engine. Select a person, search a vial/size, and open a sold quantity to see the associated orders and physical returns. All sales attribution is explicitly labelled as an estimate. The preview has no production connection or stock mutations.
