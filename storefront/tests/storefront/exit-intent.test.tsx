...
it.each(['/checkout','/checkout/thank-you','/pay/order','/subscribe/confirm','/creators','/creators/privacy'])('never interrupts transaction route %s',path=>{m.path=path;localStorage.clear();vi.useFakeTimers();window.matchMedia=vi.fn().mockReturnValue({matches:true});render(<ExitIntentModal/>);act(()=>vi.advanceTimersByTime(61000));fireEvent.mouseOut(document,{clientY:0});expect(screen.queryByText('Save 10%')).toBeNull();});
