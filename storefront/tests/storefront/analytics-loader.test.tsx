...
it('loads analytics on the creator landing page with a sanitized URL',()=>{m.path='/creators';history.replaceState({},'', '/creators?email=private@example.test#apply');const sender=vi.fn();window.gtag=sender;render(<Analytics/>);expect(screen.getByTestId('analytics-loader')).toHaveTextContent('G-TEST');expect(sender).toHaveBeenCalledWith('config','G-TEST',expect.objectContaining({page_location:'http://localhost:3000/creators'}));expect(JSON.stringify(sender.mock.calls)).not.toContain('private@example.test');});
