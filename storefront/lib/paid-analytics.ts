...
interface ClaimedPurchase { id:string;lease_token:string;payload:unknown }
...
function purchaseBody(input:unknown) {
  ...
  if (!Number.isSafeInteger(micros) || age< -300_000 || age>= (72*60-1)*60_000) throw new Error('Purchase delivery window exceeded');
  if (!Array.isArray(snapshot.events) || snapshot.events.length!==1) throw new Error('Invalid analytics event');
  const event=record(snapshot.events[0]);if(event.name!=='purchase')throw new Error('Invalid analytics event');
...
async function deliver(row:ClaimedPurchase,endpoint:string):Promise<DeliveryResult> {
  let body:string;
  try{body=JSON.stringify(purchaseBody(row.payload))}catch{return {accepted:false,permanent:true,error:'Invalid or expired purchase snapshot'}}
  try{
    ...
    return {accepted:false,permanent:response.status>=400&&response.status<500&&![408,429].includes(response.status),error:`Analytics HTTP ${response.status}`};
  }catch{
    // Provider/transport exceptions may contain the API secret in their URL.
    return {accepted:false,permanent:false,error:'Analytics transport failed'};
  }
}

/** Optional server-only worker. Retries carry the same transaction ID and paid
 * timestamp. Delivery is at least once; GA transaction deduplication is not a
 * distributed transaction or proof that an accepted event was processed. */
export async function drainPaidAnalytics(limit=25) {
  ...
  for(let i=0;i<maximum;i++){
    ...
    const row=(data as ClaimedPurchase[]|null)?.[0];if(!row)break;
