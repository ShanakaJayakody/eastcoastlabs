...
export interface AutomationMessage{id:string;to_email:string;template:string;status:OutboxState;error:string|null;created_at:string;attempt_count:number;next_attempt_at:string|null;lease_expires_at:string|null;provider_attempted_at:string|null;provider_message_id:string|null;}
/** Paged operator view; all mutations use guarded operations RPCs. */
export async function automationOverview(status:string,page=1):Promise<{summary:{status:OutboxState;count:number;oldestAt:string|null}[];rows:AutomationMessage[];total:number}>{
 ...
 let q=db.from('email_outbox').select('id,to_email,template,status,error,created_at,attempt_count,next_attempt_at,lease_expires_at,provider_attempted_at,provider_message_id',{count:'exact'});
